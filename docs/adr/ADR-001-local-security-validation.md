...
1. **題庫路徑注入風險**：系統載入題庫之路徑可直接透過 URL 的 `bank` 查詢參數或題庫選擇器傳入，若未經白名單審查，可能嘗試載入非預期的本地檔案或異常路徑。
2. **LocalStorage 資料損毀與邏輯異常**：作答進度（`examProgress`）與歷史紀錄（`examRecords`）在還原時，未經結構與欄位合法性審查便直接讀取使用，若使用者本機資料損毀或被惡意篡改，易導致陣列越界或系統崩潰。
3. **配置狀態與原型污染風險**：載入設定值時直接採用物件展開合併（`{ ...config, ...savedConfig }`），外部傳入之未知欄位可能污染內部狀態。
4. **內容安全政策 (CSP) 範圍過寬**：原 CSP 政策中允許了 `data:` URI，增加了潛在的跨站腳本 (XSS) 與敏感資料外洩攻擊面。
...
## 決策 (Decision)
我們決定在 `app.js` 與 `index.html` 中引入多層防禦性驗證機制與安全生命週期架構：

```mermaid
flowchart TD
    subgraph InputValidation [第一層：輸入與路徑防禦]
        A[URL bank 參數 / 下拉選單輸入] --> B{ALLOWED_BANKS 白名單比對}
        B -- 匹配成功 --> C[允許載入目標 JSON 題庫]
        B -- 匹配失敗 --> D[強制降級為預設 ERP 題庫]
    end

    subgraph SchemaVerification [第二層：資料格式與狀態審查]
    ...
    subgraph DefenseExecution [第三層：安全執行與防禦匯出]
